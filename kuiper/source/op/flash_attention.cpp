...
FlashAttention::FlashAttention(base::DeviceType device_type, int32_t layer_index, int32_t kv_mul,
...
base::Status FlashAttention::forward() {
  ...
  // Calculate kv_head_num from kv_mul
  int32_t kv_head_num = head_num_ / kv_mul_;

  // Call FlashAttention kernel
  kernel::get_flash_attention_kernel(device_type_)(
      query_tensor, key_tensor, value_tensor, output_tensor, head_num_, kv_head_num, head_size_,
      seq_len_, pos_, layer_index_, softmax_scale_, is_causal_, device_type_,
      cuda_config_ ? cuda_config_.get() : nullptr);

  return base::error::Success();
}

void FlashAttention::set_pos(int32_t pos) { this->pos_ = pos; }

void FlashAttention::set_layer_idx(int32_t layer_idx) { this->layer_index_ = layer_idx; }
  ...
  for (int32_t i = 0; i < 3; ++i) {
    ...
    if (!status) {
      LOG(ERROR) << "The input tensor " << std::to_string(i)
                 << " error in the FlashAttention layer.";
      return status;
    }
  }
