...
void flash_attention_kernel(const tensor::Tensor& query, const tensor::Tensor& key,
                            ...
                            bool is_causal, base::DeviceType device_type, CudaConfig* config) {
  if (device_type == base::DeviceType::kDeviceCUDA) {
#ifdef KUIPER_USE_FLASH_ATTENTION
    // Try simplified FlashAttention CUDA kernel
    LOG(INFO) << "Using simplified FlashAttention CUDA kernel";
    try {
      flash_attention_kernel_cu(query, key, value, output, head_num, kv_head_num, head_size,
                                seq_len, pos, layer_idx, softmax_scale, is_causal, config);
      return;  // Success, return early
    } catch (const std::exception& e) {
      LOG(WARNING) << "FlashAttention CUDA kernel failed: " << e.what()
                   << ". Falling back to standard MHA";
    }

    // Fallback to standard MHA if FlashAttention fails
    {
      std::shared_ptr<base::DeviceAllocator> allocator_fallback;
      if (device_type == base::DeviceType::kDeviceCPU) {
        allocator_fallback = base::CPUDeviceAllocatorFactory::get_instance();
      } else {
        allocator_fallback = base::CUDADeviceAllocatorFactory::get_instance();
      }
      tensor::Tensor score_tensor_fallback(base::DataType::kDataTypeFp32, head_num, seq_len, true,
                                           allocator_fallback);
      score_tensor_fallback.set_device_type(device_type);
      get_mha_kernel(device_type)(pos, head_num, 0, seq_len, head_num * head_size, 1, head_size,
                                  output, query, score_tensor_fallback, key, value, device_type,
                                  config);
    }
#else
