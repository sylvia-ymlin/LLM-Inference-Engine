...
base::Status LLama2Model::init(base::DeviceType device_type) {
  ...
  if (device_type == DeviceType::kDeviceCUDA) {
    ...
    cudaStreamCreate(&cuda_config_->comm_stream);
...
void LLama2Model::init_mem() {
  ...
  kv_config.max_batch_size = 1;  // Default for this demo
  kv_config.max_seq_len = config_->seq_len_;
  kv_config.layer_num = config_->layer_num_;
  kv_config.kv_head_num = config_->kv_head_num_;
  kv_config.kv_dim = config_->kv_dim_;
  kv_config.dtype = base::DataType::kDataTypeFp32;

  kv_cache_manager_ = std::make_unique<base::KVCacheManager>(device_type_, kv_config);
  kv_cache_manager_->allocate();

  void* k_ptr_raw = kv_cache_manager_->get_raw_buffer_k();
  ...
  if (device_type_ == base::DeviceType::kDeviceCUDA) {
    ...
    value_cache.set_device_type(base::DeviceType::kDeviceCUDA);
  }
