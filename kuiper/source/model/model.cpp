...
base::Status Model::init_distributed(int32_t rank, int32_t world_size,
                                     const std::string& init_method) {
  if (world_size > 1) {
    auto status = base::NcclHandler::get_instance()->Init(rank, world_size, init_method);
    if (!status) {
      return status;
    }
  }
...
std::pair<tensor::Tensor, tensor::Tensor> Model::slice_kv_cache(int32_t layer_idx,
  ...
  if (kv_cache_manager_) {
    auto [k_ptr, v_ptr] = kv_cache_manager_->get_cache_ptr(layer_idx);
    // Pre-allocated cache logic
    // Note: We need to properly wrap this into a Tensor with correct offsets if needed
    // But typically slice_kv_cache in this codebase seems to look for specific position?
    // "slice" implies just the current token's slot?
    // Original code calculated offset: int32_t cache_offset = layer_offset + token_pos *
    // config_->kv_dim_;

    int32_t cache_offset = token_pos * config_->kv_dim_;  // relative to layer start

    float* key_cache_ptr = static_cast<float*>(k_ptr) + cache_offset;
    float* val_cache_ptr = static_cast<float*>(v_ptr) + cache_offset;
